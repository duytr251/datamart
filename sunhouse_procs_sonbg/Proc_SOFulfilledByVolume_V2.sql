{{
    config(
        ...
        -- Unique key là Số SO, Số Phiếu xuất kho, và ngày ETL
...
WITH
    -- 0. Define Sources
    cicmpy_Source AS (SELECT * FROM {{ source('dp_src_shg', 'src_exactreport_cicmpy') }}),
    MPLmagaz_Source AS (SELECT * FROM {{ source('dp_warehouse_staging', 'stg_appdatashGextappdata__MPLmagaz') }}),
    orhsrg_Source AS (SELECT * FROM {{ source('dp_src_shg', 'src_exactreport_orhsrg') }}),
    orhkrg_Source AS (SELECT * FROM {{ source('dp_src_shg', 'src_exactreport_orhkrg') }}),
    orkrg_Source AS (SELECT * FROM {{ source('dp_src_shg', 'src_exactreport_orkrg') }}),
    ...
    humres_Source AS (SELECT * FROM {{ source('dp_src_shg', 'src_exactreport_humres') }}),
    addressstates_Source AS (SELECT * FROM {{ source('dp_src_shg', 'src_exactreport_addressstates') }}),
    frhkrg_Source AS (SELECT * FROM {{ source('dp_src_shg', 'src_exactreport_frhkrg') }}),
    frhsrg_Source AS (SELECT * FROM {{ source('dp_src_shg', 'src_exactreport_frhsrg') }}),
    ...
    -- 3. Lọc Incremental cho bảng Master
    FilteredMaster AS (
        SELECT *
        ...
        {% if is_incremental() %}
            -- Sử dụng pakbon_dat (ngày phiếu xuất) để lọc dữ liệu mới
            AND hk.pakbon_dat = CAST('{{ var("etl_date") }}' AS DATE)
        {% endif %}
    ),

    -- 4. CTE: Tương đương với #tblbase (Bước INSERT đầu tiên)
    tblbase AS (
        SELECT 
    ...
    -- 5. CTE: Tương đương `INSERT INTO @tblBaseParbon`
    tblBaseParbon AS (
        SELECT 
            a.*,
            f.fakdat,
    ...
    FinalData AS (
        SELECT
            ...
            END AS VARCHAR(30)) AS Norms_Computed, -- Đổi tên để tránh xung đột
            
            -- Tính StatusOrderID và StatusOrder
            CAST(CASE 
